...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="container" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <ul style={{ display: "flex", gap: "2.5rem", listStyle: "none", alignItems: "center" }} className="hidden-mobile">
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.75rem", fontWeight: 500, letterSpacing: "0.15em", textTransform: "uppercase", color: "var(--color-muted)", transition: "color 0.3s ease" }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }} className="hidden-mobile">
            ...
            <a
              ...
              style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.8rem", fontWeight: 500, color: "var(--color-primary)", letterSpacing: "0.05em" }}
            >
          ...
          >
            {[0, 1, 2].map((i) => (
              <span key={i} style={{ display: "block", width: "24px", height: "1px", background: "var(--color-text)", transition: "transform 0.3s" }} />
            ))}
          </button>
        </div>
      </nav>
      ...
      {/* Mobile Dropdown */}
      {menuOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 99, background: "rgba(17,17,17,0.98)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "2.5rem" }}>
          <button onClick={closeMenu} style={{ position: "absolute", top: "1.5rem", right: "1.5rem", background: "none", border: "none", color: "var(--color-muted)", fontSize: "1.5rem", cursor: "pointer" }}>
            ✕
          </button>
          {NAV_LINKS.map((link) => (
            <a key={link.href} href={link.href} onClick={closeMenu} style={{ fontFamily: "'Playfair Display', serif", fontSize: "2rem", fontStyle: "italic", color: "var(--color-text)", transition: "color 0.3s" }}>
              {link.label}
            </a>
          ))}
